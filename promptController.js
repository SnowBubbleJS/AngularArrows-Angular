...
var promptModel = {
  array: [
    ...
    "Now onto a bit of coding, write <code>ng-app</code> into the 'html' tag.",
    "Congratulations, you've initiated your first Angular application, not too bad right? Next, let's add an input box within your body like so, <input type='text' ng-model='yourName'>. What is ng-model you ask? ng-model is an Angular directive and what the ng-model directive does is that it binds things like an input or textarea to your applications data.",
    ...
    "Now that we've built the foundation of our application, let's build a controller using the Angular Style Guide by John Papa.",
    ...
    "In the HTML file, within the <div> you created for the ng-controller, write the following Angular Expressions. <code>{{name}}</code> <code>{{age}}</code> <code>{{occupation}}</code>.",
    "Great job! But, doesn't it seem like {{name}}, {{age}}, {{occupation}} seem too broad? What if we were creating an application that used those properties a lot? Well we can fix that using john papa's style guide using controllerAs syntax.",
    "In the html instead of <body ng-controller='myController'>, we will be using <body ng-controller='myController as myCtrl'>, make this adjustment into your index.html. Using this syntax in the view, angular will force you to use 'dotted' objects in the view, which will prevent reference issues e.g using {{myCtrl.name}} makes it a lot more understandable/readable than {{name}}, you might be using name in a lot more places, but it'll be difficult to catch at first. For more information about this method, please visit https://github.com/johnpapa/angular-styleguide#controllers.",
    ...
    "As mentioned earlier, since we're using the controllerAs syntax, we can now show these changes in the view using: <code>{{myCtrl.name}}</code> <code>{{myCtrl.age}}</code> <code>{{myCtrl.occupation}}</code> This will also create two-way binding between our view and our controller, meaning that one change to either will affect the other."
  ],
